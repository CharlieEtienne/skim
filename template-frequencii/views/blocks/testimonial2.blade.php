            ...
            @if(isset($testimonials) && count($testimonials) > 0)
                @foreach($testimonials as $testimonial)
                    <div class="bg-gray-50 px-8 py-10 rounded-md">
                        <p class="font-normal text-gray-500 text-md mb-4">{{ $testimonial['testimonial_text'] ?? 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.' }}</p>
                        <h6 class="font-semibold text-gray-500 text-md">{{ $testimonial['author_name'] ?? 'Stephan Clark' }} <span class="font-medium text-gray-300 text-sm">{{ $testimonial['author_title'] ?? '- CEO at EarlyBird' }}</span></h6>
                    </div>
                @endforeach
            @else
                <!-- Fallback for when no testimonials are defined -->
                @for($i = 0; $i < 6; $i++)
                    <div class="bg-gray-50 px-8 py-10 rounded-md">
